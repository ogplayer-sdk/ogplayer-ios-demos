...
/// - "FairPlay": the multi-DRM test vector the DRM demo shares — its
///   entitlement explicitly allows licence persistence
...
struct DownloadsDemo: View {
    ...
    /// "title · state · percent · licence info".
    ...
    private func item(for stream: DownloadStream) -> OGMediaItem? {
        ...
        case .fairplay:
            // The multi-DRM test vector from DrmDemo — the token's
            ...
            return OGMediaItem(
                ...
                posterUrl: URL(string: "https://media.ogplayer.tv/posters/tos-mech.jpg"),
                drm: FairPlayConfig(
                    certificateURLString: sharedCertificateURL,
                    licenseServerURLString: sharedLicenseURL,
                    licenseHeaders: ["X-AxDRM-Message": persistentToken],
...
// Public multi-DRM test vector — the same certificate, licence server
// and JWT DrmDemo's "Multi-DRM (shared)" chip uses. Public test credentials;
// the entitlement message allows licence persistence.
private let sharedCertificateURL = "https://tools.axinom.com/FPScert/fairplay.cer"
private let sharedLicenseURL = "https://drm-fairplay-licensing.axprod.net/AcquireLicense"
private let persistentToken =
